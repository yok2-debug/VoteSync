...
export function VoterImportDialog({ open, onOpenChange, data, categories }: VoterImportDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [validatedData, setValidatedData] = useState<ValidatedRow[]>([]);
  const { voters: existingVoters, refreshData } = useDatabase();
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const response = await fetch('/api/voters/import', {
        ...
        body: JSON.stringify({ voters: dataToImport }),
      });
