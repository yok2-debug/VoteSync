...
export function RoleTable({ roles }: RoleTableProps) {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const isRoleInUse = adminUsers.some(user => user.roleId === selectedRole.id);
  
      if (isRoleInUse) {
        throw new Error('Tidak dapat menghapus peran. Peran ini sedang digunakan oleh satu atau lebih pengguna.');
      }
  
      await remove(ref(db, `roles/${selectedRole.id}`));
